...
class RewriteDescriptionBody(BaseModel):
    variant_index: int = Field(default=0, ge=0, le=3)
    property_id: Optional[str] = None


@router.post("/jobs/{job_id}/rewrite-description")
async def rewrite_description(
    job_id: str,
    body: RewriteDescriptionBody,
    user=Depends(get_current_user),
) -> Dict[str, str]:
    """AL riscrive la descrizione dell'annuncio in coerenza con lo stile del render scelto."""
    db = Database.get()
    doc = await db.virtual_staging_jobs.find_one({"id": job_id, "user_id": user["id"]})
    if not doc:
        raise HTTPException(404, "Job non trovato")
    ...
    if agency_id:
        q["agency_id"] = agency_id
    prop = await db.properties.find_one(q, {"_id": 0, "documents": 0, "photos": 0})
    if not prop:
        raise HTTPException(404, "Immobile non trovato")

    variants = doc.get("variants") or []
    style_key = variants[body.variant_index]["style"] if body.variant_index < len(variants) else doc.get("style", "modern")
    style_label = STYLES.get(style_key, {}).get("label", style_key)
    room_label = ROOM_LABELS.get(doc.get("room_type", "living"), doc.get("room_type"))
    ...
    try:
        ...
        ctx = {
            "titolo": prop.get("title"),
            "descrizione_attuale": (prop.get("description") or "")[:2000],
            "tipologia": prop.get("property_type"),
            "operazione": prop.get("operation"),
            "città": prop.get("city"),
            "zona": prop.get("zone"),
            "prezzo": prop.get("price"),
            "superficie_mq": prop.get("surface_sqm"),
            "locali": prop.get("rooms"),
            "classe_energetica": (prop.get("energy") or {}).get("energy_class"),
            "stile_render": style_label,
            "stanza_arredata": room_label,
        }
        chat = LlmChat(
            api_key=api_key,
            session_id=f"staging-desc-{job_id[:8]}",
            system_message=(
                "Sei AL, copywriter immobiliare di OMNIA. Riscrivi la descrizione dell'annuncio "
                "in italiano (120-180 parole) valorizzando lo stile del virtual staging appena creato "
                f"(stile {style_label}, {room_label}). Regole: usa SOLO i dati forniti, non inventare "
                "caratteristiche o numeri; tono professionale ma caldo; menziona che le immagini di "
                "arredamento sono render virtuali indicativi. Rispondi SOLO con la descrizione finale, "
                "senza titoli né spiegazioni."
            ),
        ).with_model("gemini", "gemini-3-flash-preview")
        raw = await asyncio.wait_for(chat.send_message(UserMessage(text=_json.dumps(ctx, ensure_ascii=False, default=str))), timeout=30)
        text = str(raw).strip()
        if not text:
            raise RuntimeError("empty LLM response")
        return {"description": text, "style": style_key}
    except HTTPException:
        raise
    except Exception as e:
        logger.warning("rewrite-description failed: %s", e)
        raise HTTPException(503, "AL non disponibile al momento, riprova")
